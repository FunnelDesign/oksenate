...
namespace Drupal\senate_calendar\Form;
...
class SenateCalendarForm extends ConfigFormBase {
  ...
  public function buildForm(array $form, FormStateInterface $form_state) {
    $config = $this->configFactory->get('senate_calendar.settings');

    $request_time = \Drupal::time()->getRequestTime();
    $next_execution = $this->state->get('senate_calendar.next_execution');
    ...
    $args = [
      '%time' => date('c', $this->state->get('senate_calendar.next_execution')),
    ...
    $form['cron_run']['cron_get_all'] = [
      '#type' => 'checkbox',
      '#title' => $this->t("Get Bills from files regardless of whether file has been updated."),
      '#default_value' => FALSE,
    ];
  ...
  public function cronRun(array &$form, FormStateInterface &$form_state) {
    ...
    if (!empty($cron_reset)) {
      $this->state->set('senate_calendar.next_execution', 0);
    }
    if (!empty($cron_get_all)) {
      $this->state->set('senate_calendar.prev_execution', 0);
    }
    if (!empty($cron_update_all)) {
      $this->state->set('senate_calendar.update_all', TRUE);
    }
    if (!empty($directory)) {
      $this->state->set('senate_calendar.directory', $directory);
    }

    // Use a state variable to signal that cron was run manually from this form.
    $this->state->set('senate_calendar_show_status_message', TRUE);
    if ($this->cron->run()) {
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->configFactory->getEditable('senate_calendar.settings')
  ...
  protected function getEditableConfigNames() {
    return ['senate_calendar.settings'];
  }
