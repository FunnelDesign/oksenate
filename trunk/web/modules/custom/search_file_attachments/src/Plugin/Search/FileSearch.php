...
class FileSearch extends SearchPluginBase implements AccessibleInterface, SearchIndexingInterface {
   ...
   */
  public function __construct(array $configuration, $plugin_id, $plugin_definition, Connection $database, EntityTypeManagerInterface $entity_type_manager, Config $search_settings, Config $module_settings, LanguageManagerInterface $language_manager, AccountInterface $account = NULL) {
  ...
  public function indexClear() {
    // All NodeSearch pages share a common search index "type" equal to
    // the plugin ID.
    \Drupal::service('search.index')->clear($this->getPluginId());
  }
  ...
  protected function indexFile(EntityInterface $file) {
    ...
    foreach ($languages as $language) {
      ...
      \Drupal::service('search.index')->index($this->getPluginId(), $file->id(), $language->getId(), $content);
    }
  }
